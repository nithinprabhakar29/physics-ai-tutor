...
class PhysicsRAGSystem:
    def __init__(self, db_path: str = os.getenv("DB_PATH", "./physics_knowledge_db"),
    ...
    def extract_text_from_pdf(self, pdf_path: str) -> str:
        ...
        # ✅ This now runs correctly after PyPDF2 logic
        if len(text.strip()) < 100:
            print("⚠️ Trying OCR for image-based PDF...")
            try:
        ...
        return text  # ✅ Final return now after both attempts

    def extract_diagram_regions(self, pdf_path: str) -> dict:
        diagrams = {}
        try:
            pages = convert_from_path(pdf_path, dpi=300,poppler_path = os.getenv("POPPLER_PATH"))
            for i, page in enumerate(pages):
                img = np.array(page)
                ...
                for j, cnt in enumerate(contours):
                    ...
                    if w > 120 and h > 120:
                        ...
                        filename = f"{os.path.splitext(os.path.basename(pdf_path))[0]}_pg{i + 1}_img{j + 1}.png"
                        os.makedirs("diagrams", exist_ok=True)
                        Image.fromarray(crop).save(os.path.join("diagrams", filename))
                        diagrams[i + 1] = filename
                        break  # Only one image per page for now
        except Exception as e:
...
Generate 3 follow-up questions:"""
    ...
    def process_book(self, pdf_path: str, book_info: Dict):
        ...
        chunks = self.chunk_text(full_text, chunk_size=1500, overlap=300)
        diagrams_by_page = self.extract_diagram_regions(pdf_path)
        ...
        for i, chunk in enumerate(chunks):
            ...
            if (i+1) in diagrams_by_page:
                metadata['diagram_filename'] = diagrams_by_page[i+1]
...
def setup_books(rag_system, folder_path: str):
    ...
    # JEE Reference Books
    jee_books = [
        #{"file": "Wiley's Physics JEE Main Practice Problems.pdf", "name": "Wiley JEE Main Practice Problems",
         #"source_type": "jee_questions", "author": "Wiley", "identifier": "wiley_questions"},
    ...
    while True:
        ...
        if choice == '4':
            ...
            if topic:
                ...
                difficulty = input("Difficulty (JEE/NCERT/MIXED): ").strip().lower()
                if difficulty.lower() not in ['jee','ncert', 'mixed']:
                    difficulty = 'mixed'

                print("\nGenerating quiz...")
                result = rag.quiz_generation(topic, num_questions, difficulty)

                if 'error' in result:
                    print(f"Error: {result['error']}")
                else:
                    print(f"\n📝 Quiz: {result['topic']} ({result['quiz']} difficulty)")
                    print("=" * 50)
